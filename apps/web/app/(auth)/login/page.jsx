...
// Composant pour afficher quand l'utilisateur est déjà connecté - demande toujours le mot de passe
function AlreadyLoggedIn({ session, onSignOut }) {
  ...
  const handleSignOut = async () => {
    ...
    try {
      ...
      // Forcer le rafraichissement de la page pour afficher le formulaire de login
      router.refresh();
      onSignOut();
    } catch (error) {
  ...
  // Vérifier le mot de passe avant d'accéder au dashboard
  const handleAccessDashboard = async (e) => {
    e.preventDefault();
    setConfirmError("");
    ...
    try {
      // Re-authentifier avec le même email et le mot de passe fourni
      ...
      if (result?.error) {
        ...
        // Mot de passe correct, accéder au dashboard
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-center">
        <div className="flex items-center justify-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="font-medium text-blue-700">{session?.user?.name || session?.user?.email}</p>
          </div>
        </div>
      </div>

      {/* Formulaire avec mot de passe toujours visible */}
      <form onSubmit={handleAccessDashboard} className="space-y-5">
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              readOnly
              className="block w-full pl-10 pr-4 py-3 border border-gray-200 rounded-lg text-gray-700 bg-gray-50 cursor-not-allowed"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <p className="mt-2 text-xs text-gray-500">
            Pour des raisons de securite, veuillez confirmer votre mot de passe.
          </p>
        </div>
        ...
        <div className="space-y-3">
          <button
            type="submit"
            disabled={confirming}
            className="w-full flex items-center justify-center gap-2 py-3 px-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl hover:from-blue-700 hover:to-blue-800 transform hover:-translate-y-0.5 transition-all duration-200 disabled:opacity-50 disabled:transform-none"
          >
            {confirming ? (
              <>
