...
export default function EditTreatyPage({ params }) {
  const router = useRouter();
  const { id } = use(params);
  const { setPageTitle } = usePageTitle();
  const [loading, setLoading] = useState(false);
  const [loadingTreaty, setLoadingTreaty] = useState(true);
  ...
  useEffect(() => {
    if (formData.title) {
      document.title = `Modifier - ${formData.shortTitle || formData.title} | ANAPI`;
      setPageTitle(`Modifier - ${formData.shortTitle || formData.title}`);
    }
    return () => setPageTitle(null);
  }, [formData.title, formData.shortTitle, setPageTitle]);
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          treatyType: data.treatyType || "BIT",
          status: data.status || "NEGOTIATING",
          partnerCountries: data.partnerCountries || [],
          regionalOrganization: data.regionalOrganization || "",
          negotiationStartDate: formatDateForInput(data.negotiationStartDate),
          signedDate: formatDateForInput(data.signedDate),
          ratifiedDate: formatDateForInput(data.ratifiedDate),
          entryIntoForceDate: formatDateForInput(data.entryIntoForceDate),
          expiryDate: formatDateForInput(data.expiryDate),
          duration: data.duration || "",
          autoRenewal: data.autoRenewal || false,
          renewalPeriod: data.renewalPeriod || "",
          keyProvisions: data.keyProvisions || [],
          investorBenefits: data.investorBenefits || [],
          coveredSectors: data.coveredSectors || [],
          exclusions: data.exclusions || [],
          disputeResolution: data.disputeResolution || "",
          treatyTextUrl: data.treatyTextUrl || "",
          notes: data.notes || "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        await Swal.fire({
          icon: "success",
          title: "Traité modifié",
          text: "Les modifications ont été enregistrées",
          confirmButtonColor: "#2563eb",
          timer: 2000,
          timerProgressBar: true,
        });
        router.push(`/business-climate/treaties/${id}`);
      } else {
      ...
      Swal.fire({
        icon: "error",
        title: "Erreur",
        text: "Erreur lors de la modification du traité",
        confirmButtonColor: "#2563eb",
      });
    } finally {
      setLoading(false);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 p-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Link
          href={`/business-climate/treaties/${id}`}
        ...
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            {formData.shortTitle || formData.title}
          </p>
        </div>
      </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6">
          ...
          <div className="flex gap-2 mb-3">
            <input
              ...
              className="flex-1 px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              onKeyPress={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-6">
          ...
          <div className="flex gap-2 mb-3">
            <input
              ...
              placeholder="Ajouter une disposition..."
              className="flex-1 px-4 py-2.5 border border-gray-200 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              onKeyPress={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
