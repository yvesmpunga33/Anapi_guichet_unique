...
function DeductionFormDialog({ open, onClose, deduction, onSave, categories, intl }) {
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-white">
                {deduction ? intl.formatMessage({ id: 'hr.deductions.editDeduction', defaultMessage: 'Modifier la reduction' }) : intl.formatMessage({ id: 'hr.deductions.newGeneralDeduction', defaultMessage: 'Nouvelle Reduction Generale' })}
              </h2>
              {reference && (
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div className="sm:col-span-2">
              <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                {intl.formatMessage({ id: 'hr.deductions.category', defaultMessage: 'Categorie' })} *
              </label>
              <select
                ...
                required
              >
                <option value="">{intl.formatMessage({ id: 'hr.deductions.selectCategory', defaultMessage: 'Selectionner une categorie' })}</option>
                {categories.map((cat) => (
              ...
              <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                {intl.formatMessage({ id: 'hr.deductions.categoryHelp', defaultMessage: 'La reduction sera appliquee a tous les employes de cette categorie' })}
              </p>
            </div>
            ...
            <div>
              <label className="mb-2 block text-sm font-medium text-gray-700 dark:text-gray-300">
                {intl.formatMessage({ id: 'hr.deductions.deductionName', defaultMessage: 'Nom de la reduction' })} *
              </label>
              <input
                type="text"
                value={formData.nom}
                onChange={(e) => handleChange('nom', e.target.value)}
                placeholder="Ex: Cotisation syndicale"
                className="w-full rounded-xl border border-gray-200 px-4 py-3 focus:border-[#D4A853] focus:outline-none focus:ring-2 focus:ring-[#D4A853]/20 dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                required
              />
            </div>
...
export default function GeneralDeductionsPage() {
  const intl = useIntl();
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [deductionsRes, categoriesRes] = await Promise.all([
        getDeductions({
          page: page + 1,
          limit: rowsPerPage,
          search,
      ...
      if (deductionsRes.success) {
        ...
        const generalDeductions = data.filter((d) => d.categoryId && !d.employeeId);
        setDeductions(generalDeductions);
        setTotal(generalDeductions.length);
      }
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">
              {intl.formatMessage({ id: 'hr.deductions.generalDeductions', defaultMessage: 'Reductions Generales' })}
            </h1>
            <p className="text-gray-600 dark:text-gray-400">
              {intl.formatMessage({ id: 'hr.deductions.generalDeductionsSubtitle', defaultMessage: "Reductions par categorie d'employes" })}
            </p>
          </div>
        </div>
        <button
                ...
                <tr>
                  <td colSpan={8} className="px-6 py-12 text-center">
                    ...
                    <p className="mt-2 text-gray-600 dark:text-gray-400">
                      {intl.formatMessage({ id: 'hr.deductions.noGeneralDeductions', defaultMessage: 'Aucune reduction generale trouvee' })}
                    </p>
                  </td>
                </tr>
              ) : (
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        <span
                          ...
                          style={{ backgroundColor: deduction.category?.couleur || '#6b7280' }}
                        >
                          {deduction.category?.code?.[0] || 'C'}
                        </span>
                        <span className="text-gray-900 dark:text-white">{deduction.category?.nom}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="font-medium text-gray-900 dark:text-white">{deduction.nom}</span>
                    </td>
